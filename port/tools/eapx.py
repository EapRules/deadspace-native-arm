...
ROOT_KEYS = {
    "schema", "id", "version", "title", "abi_order", "input", "extract",
    "hooks", "validate", "commit", "marker", "space", "log", "placeholder",
}
SEMANTIC_KEYS = {"extract", "hooks", "validate", "commit", "abi_order", "input", "schema"}
...
class Tree:
    ...
    Some games are not distributed as a zip at all -- Dead Space arrives as a
    7-Zip archive of an already-exploded APK, and neither Python's stdlib nor
    the firmware can read 7z. Rather than carry a decoder, the user unpacks on
    their PC and copies the folder; from here it behaves exactly like an
    archive, so every rule kind works against it unchanged.
    """
...
def validate_output_checks(root, recipe, abi, digests):
    ...
    for check in recipe.output_checks:
        relative = template(check["path"], abi)
        path = os.path.join(root, relative)
        spec = check["spec"]
        if set(spec) & FILE_VALIDATORS:
            if not is_regular_file(path):
                raise ValidationError("validate: missing file %s" % relative)
            sha256, _crc, size = digests.file(path)
            with open(path, "rb") as stream:
                header = stream.read(64)
            reason = check_file_spec(spec, size, sha256, header, abi, relative)
            if reason:
                raise ValidationError("validate %s: %s" % (relative, reason))
        else:
            if not os.path.isdir(path):
                raise ValidationError("validate: missing directory %s" % relative)
